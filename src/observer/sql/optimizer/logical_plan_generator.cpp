...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, std::unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  RC rc = create_plan(filter_stmt, predicate_oper);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to create update's predicate operator. rc=%s", strrc(rc));
    return rc;
  }
  ...
  logical_operator.reset(new UpdateLogicalOperator(table, update_stmt->fields(), update_stmt->values(), update_stmt->value_amount()));
